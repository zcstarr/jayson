 ...
 */
ClientBrowser.prototype.request = async function(method, params, id, callback) {
  const self = this;
  let request = null;
  ...
  this.callServer = promisify(this.callServer);
 ...
 * @private
 */
ClientBrowser.prototype._parseResponse = async function(err, responseText) {
  if(err) {
    throw err
  }
  ...
  if(callback.length === 3) {
    ...
    if(isArray(response)) {
      ...
      return response.filter(negate(isError));
